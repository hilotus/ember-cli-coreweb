...
export default Ember.Object.extend({
  ...
  */
  findById: function(clazz, id) {
    ...
    return adapter.find(clazz, id).then(function(json){
      return Ember.RSVP.resolve(self.push(clazz, json));
    }, function(reason){
      return Ember.RSVP.reject(self.findByIdFailure(reason));
    });
  },
  ...
  * Find By Ids
  */
  findByIds: function(clazz, ids) {
    if (!$.isArray(ids)) {
      throw new Error('ids must be an array.');
    }

    clazz = this.__getModelClazz(clazz);
    var typeKey = clazz.typeKey,
      records = [],
      self = this;
    ...
    return this.container.lookup('adapter:application').findByIds(clazz, ids).then(function(json){
      json.results.forEach(function(r){
        var record = self.push(clazz, r);
        records.pushObject(record);
      });
  ...
  __normalize: function(record, data) {
    ...
    for (var key in data) {
      if (!Ember.isNone(schema.belongTo[key])) {
        ...
        record.set(key, this.__normalizeEmbedded(data[key]));
  ...
  * value: model column value
  */
  __normalizeHasMany: function(record, typeKey, key, value) {
    this.findByIds(typeKey, value).then(function(records){
  ...
  */
  __normalizeEmbedded: function(data) {
    if ($.isPlainObject(data)) {  // check value is json data.
      var value = Ember.Object.create();
      for (var key in data) {
        value.set(key, this.__normalizeEmbedded(data[key]));
      }
      return value;
    } else {
      return data;
  ...
  */
  __push: function(clazz, json, record) {
    ...
    if (Ember.isNone(record)) {
      if (!__cache__[clazz.typeKey][json.id]) {  // Only there is no cache, create a nre record.
        record = clazz.create();
      } else {
